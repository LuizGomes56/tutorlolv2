...
pub fn check_bin_packer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for champion in ChampionId::VALUES {
        for metadata in champion.abilities() {
            ...
            result.push_str(&format!("{champion:?}::{ability_id:?}<br>"));
            result.push_str(&render_ch_formula(champion, ability_id)?);
...
pub const fn class_id(class: RenderClass) -> u8 {
    match class {
        RenderClass::Number => Class::Number as u8,
        RenderClass::Context => Class::Variable as u8,
...
static FORMULA_BYTES: &[u8] = include_bytes!("../../packer.bin");

pub fn render_ch_formula(
    champion: ChampionId,
    ...
    let html = db.render_formula_html(
        ...
        |local| {
            ...
            format!(
                "{}_{}",
                champion.debug().to_lowercase(),
                ability.kind.discriminant().to_lowercase()
            )
        },
        class_id,
    )?;
