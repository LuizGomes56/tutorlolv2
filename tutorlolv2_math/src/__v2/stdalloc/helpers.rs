...
pub fn get_abilities_data(
    ...
    level: u8,
) -> DamageKind<AbilityLike> {
    let mut metadata = Vec::with_capacity(ability_cache.len());
    let mut damages = Vec::with_capacity(ability_cache.len());
...
#[inline]
pub fn get_runes_data(runes: &SetU32, level: u8) -> DamageKind<RuneId> {
    let mut metadata = Vec::with_capacity(runes.len());
    let mut damages = Vec::with_capacity(runes.len());
...
#[inline]
pub fn get_items_data(items: &SetU32, attack_type: AttackType, level: u8) -> DamageKind<ItemId> {
    let mut metadata = Vec::with_capacity(items.len());
    let mut damages = Vec::with_capacity(items.len());
...
pub fn eval_damage<const N: usize, T>(
    ...
    let mut result = Vec::with_capacity(closures.len());
