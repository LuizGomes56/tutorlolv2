...
#[inline]
pub fn get_abilities_data<'a>(
    arena: &'a Bump,
    ability_cache: &'static [(AbilityLike, CachedChampionAbility)],
    ability_levels: AbilityLevels,
    level: u8,
) -> DamageKind<'a, AbilityLike> {
    let metadata =
        unsafe { alloc_uninit_slice::<TypeMetadata<AbilityLike>>(arena, ability_cache.len()) };
    let damages = unsafe { alloc_uninit_slice::<DamageClosure>(arena, ability_cache.len()) };
    for (index, (key, value)) in ability_cache.iter().enumerate() {
        let level = match key {
...
pub unsafe fn alloc_uninit_slice<'a, T>(arena: &'a Bump, len: usize) -> &'a mut [T] {
    unsafe {
        assert!(size_of::<T>() != 0, "Slice can't have zero length");
        let bytes = len.unchecked_mul(size_of::<T>());
        let layout = Layout::from_size_align_unchecked(bytes, align_of::<T>());
        let ptr_u8 = arena.alloc_layout(layout);
...
#[inline]
pub fn get_runes_data<'a>(arena: &'a Bump, runes: &'a SetU32, level: u8) -> DamageKind<'a, RuneId> {
    let metadata = unsafe { alloc_uninit_slice::<TypeMetadata<RuneId>>(arena, runes.len()) };
    // let mut metadata = arena.slice_(runes.len());
    let damages = unsafe { alloc_uninit_slice::<DamageClosure>(arena, runes.len()) };
    for (index, rune_number) in runes.iter().enumerate() {
...
#[inline]
pub fn get_items_data<'a>(
    arena: &'a Bump,
    items: &'a SetU32,
    attack_type: AttackType,
    level: u8,
) -> DamageKind<'a, ItemId> {
    let metadata = unsafe { alloc_uninit_slice::<TypeMetadata<ItemId>>(arena, items.len()) };
    let damages = unsafe { alloc_uninit_slice::<DamageClosure>(arena, items.len()) };
...
pub fn get_enemy_state(state: EnemyState, shred: ResistShred, earth_dragons: u8) -> EnemyFullState {
    ...
    EnemyFullState {
        ...
        armor_values: unsafe { std::mem::transmute(armor_values) },
        magic_values: unsafe { std::mem::transmute(magic_values) },
        // #![manual_impl]
...
pub fn eval_damage<'a, T>(
    ...
    let result = unsafe { alloc_uninit_slice::<RangeDamageI32>(arena, closures.len()) };
