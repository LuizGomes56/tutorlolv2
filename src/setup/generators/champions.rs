...
pub fn run_generator_file(path_name: &str) -> Result<(), SetupError> {
    let result: CdnChampion = read_json_file(path_name)
        .map_err(|e| SetupError(format!("Failed to read '{}': {:#?}", path_name, e)))?;
    let name: &str = extract_file_name(Path::new(path_name));
    let name_lower: String = name.to_lowercase();
    let champion: Option<Champion> = try_run_generator(&name_lower, result);
...
pub async fn create_generator_files() -> Result<(), SetupError> {
    ...
    let generator_target: &'static str = "src/generators";
    ...
    for (_, champion_id) in champion_names {
        futures.push(tokio::spawn(async move {
            let path_name: String =
                format!("{}/{}.rs", generator_target, champion_id.to_lowercase());
...
"#,
            ...
            let _ = write_to_file(&path_name, autogenerated_content.trim().as_bytes())
                .map_err(|e: SetupError| eprintln!("fn[create_generator_files]: {:#?}", e));
        }));
    }
...
fn transform_ability(key: &str, abilities: &[CdnAbility]) -> Vec<String> {
    let mut generator_keybinds: Vec<String> = Vec::<String>::new();
