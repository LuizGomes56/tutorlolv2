...
use crate::model::{
    base::{
        AttackType, BasicStats, ComparedItem, CurrentPlayerLike, DamageLike, DamageMultipliers,
        EnemyFullStats, FullStats, InstanceDamage, RealResists, SelfFullStats, Stats,
    },
...
pub(super) fn get_comparison_total_damage<T>(prev: &DamageLike<T>, next: &mut DamageLike<T>) -> f64
where
    T: Eq + Hash,
{
    let mut sum = 0f64;
